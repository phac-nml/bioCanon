...
if __name__ == '__main__':
    ...
    if cp.stderr:
        print("There was an error attempting to run Biohansel (command 'hansel -V'). Please confirm that Biohanel is installed and you have permission to run it.")
        exit()
    
    # Run biohansel on the inputs    
    cp = subprocess.run(['hansel','-s',inputs.in_scheme,'-D',inputs.in_seq_data,'--output-kmer-results',biohansel_kmer_fn,'--output-summary',biohansel_summary_fn,'--force',inputs.biohansel_arguments],stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=False)
    if cp.stderr:
    ...
    for el in dfk_values.index:
        ...
        if pos_line[:8] == 'negative':
            ...
            if position not in pdict:
                print(f"Invalid tile in {biohansel_kmer_fn}: {position} not recognized as a position from {inputs.in_scheme}.")
                continue
